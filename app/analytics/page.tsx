      ...
      <main className="pt-16 pb-12 px-8 max-w-screen-2xl mx-auto flex gap-8">
        <aside className="hidden lg:flex h-[calc(100vh-140px)] w-64 sticky top-28 flex-col p-6 gap-y-4 bg-white border border-zinc-100 font-manrope text-sm rounded-xl">
          <nav className="flex flex-col gap-1">
            <a className="flex items-center gap-3 p-3 text-zinc-900 font-semibold bg-zinc-50 rounded-xl" href="#">
        ...
        <section className="flex-1">
          <header className="mb-12 flex justify-between items-end">
            ...
            <div className="flex gap-3 bg-white p-1.5 rounded-xl shadow-sm border border-zinc-100">
              <button onClick={() => setActiveRange('24h')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${activeRange === '24h' ? 'bg-zinc-900 text-white' : 'text-zinc-500'}`}>24H</button>
              <button onClick={() => setActiveRange('7d')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${activeRange === '7d' ? 'bg-zinc-900 text-white' : 'text-zinc-500'}`}>7D</button>
              <button onClick={() => setActiveRange('30d')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${activeRange === '30d' ? 'bg-zinc-900 text-white' : 'text-zinc-500'}`}>30D</button>
            </div>
